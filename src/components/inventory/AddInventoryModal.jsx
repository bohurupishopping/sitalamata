...
export default function AddInventoryModal({ isOpen, onClose, onSubmit, categories }) {
  ...
  useEffect(() => {
    if (newItem.category) {
      const q = query(collection(db, 'items'), where('category', '==', newItem.category))
  ...
  const handleCategoryChange = (e) => {
    ...
    setNewItem({ ...newItem, category: categoryId, name: '' })
  }
  ...
  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center ${!isOpen && 'hidden'}`}>
      <div className="bg-white p-6 rounded-lg w-full max-w-md mx-4">
        <h2 className="text-xl font-bold mb-4 text-gray-800">Add New Inventory</h2>
